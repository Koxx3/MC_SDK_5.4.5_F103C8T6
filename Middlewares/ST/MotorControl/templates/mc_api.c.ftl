...
/** 
 * @brief returns Iq and Id current values in High Frequency Injection context for Motor 1, in qd_t format 
 */
__weak qd_t MC_GetIqdHFMotor1(void)
{
	return MCI_GetIqdHF( pMCI[M1] );
}
</#if>
 ...
 */
__weak STC_Modality_t MC_GetControlModeMotor2(void)
{
...
/** 
 * @brief returns Iq and Id current values in High Frequency Injection context for Motor 2, in qd_t format 
 */
__weak qd_t MC_GetIqdHFMotor2(void)
{
	return MCI_GetIqdHF( pMCI[M2] );
}
</#if>
 ...
 * Calling this function has no effect when either flux weakening region is entered or MTPA is enabled.
 */
__weak void MC_SetIdrefMotor2( int16_t hNewIdref )
{
...
/**
 * @brief Start the Encoder Alignment procedure for Motor 2, if possible
 *
 *  This command is used to trigger the encoder alignment procedure.
 * If Motor 2's state machine is in #IDLE state, the command is immediately
 * executed and true is returned. Otherwise, the command is discarded and false
 * is returned.  
 *
 * @note The MC_AlignEncoderMotor2() command only triggers the alignment procedure: 
 * It moves Motor 1's state machine from the #IDLE to the #IDLE_ALIGNMENT state and 
 * then returns. It is not blocking the Application until the motor is indeed aligned. 
 * The Application can query Motor 2's state machine to check if the alignment  
 * procedure has completed: it is the case when the state machine is back to the #IDLE 
 * state.
 *
 * See MC_GetSTMStateMotor2() for more details.
 *
 * @retval true if the command is successfully executed, false otherwise.
 */
__weak bool MC_AlignEncoderMotor2(void)
{
	return MCI_EncoderAlign( pMCI[M2] );
}
</#if>
 ...
 */
__weak State_t  MC_GetSTMStateMotor2(void)
{
