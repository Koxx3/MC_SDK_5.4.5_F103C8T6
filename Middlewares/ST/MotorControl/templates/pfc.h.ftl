...
<#if CondFamily_STM32F3>
    TIM_TypeDef * TIMx_2;                 /**< Timer used for PFC (Registers base address) */    
<#elseif CondFamily_STM32F1>      
    ...
    uint16_t hNominalCurrent;            /**< Output nominal current in s16A */
...
typedef struct
{
  ...
  uint16_t hCurrentReferenceAmpl;        /**< DC output current, amplitude reference level */
  uint16_t hCurrentReference;            /**< DC output current, reference level. */
...
/* It is called to move the state macchine to the HW fault state. */
