...
/* Exported types ------------------------------------------------------------*/
<#-- ST MCWB monitoring usage management (used when MC.SERIAL_COMMUNICATION == true) -->
<#if MC.DRIVE_TYPE == "FOC" || MC.SERIAL_COMMUNICATION == true || MC.DAC_FUNCTIONALITY == true><#-- TODO: PBo, Is this condition (MC.DRIVE_TYPE == "FOC") really needed ? -->
/**
...
typedef enum
{
  ...
  DAC_USER2
	...
	<#-- Only for the F7 family. For other families, this flag is defined in 
		 a CMSIS header file if the target MCU supports 2 DAC Channels -->
/* Enables both F7 DAC channels */
...
#define CTRBDID ${MC.CTRBDID}
#define PWBDID ${MC.PWBDID}
<#if MC.MOTOR_PROFILER == true>
#include "UID.h"
#define MC_UID UID
<#else>
#define MC_UID ${MC.UID}
</#if>

<#if MC.DUALDRIVE == true>
#define PWBDID2 ${MC.PWBDID2}
</#if>
...
<#elseif MC.DRIVE_TYPE == "SIX_STEP">
  MC_Handle_t ** pMCI;             /*!< Pointer on MC 6-Step interface list.*/
	<#if MC.SERIAL_COMMUNICATION == false>
  UART_HandleTypeDef * pUART;      /**< Pointer on the UART HAL Handle */
	</#if>
</#if>
...
<#elseif MC.DRIVE_TYPE == "SIX_STEP">
void UI_Init(UI_Handle_t *pHandle, uint8_t bMCNum, MC_Handle_t** pMCI, uint32_t* pUICfg);
</#if>
