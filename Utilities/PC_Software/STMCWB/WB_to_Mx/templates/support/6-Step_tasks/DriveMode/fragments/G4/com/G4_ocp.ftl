...
<#function cs_over_current_prot motor phases = [""]>
    ...
    <#local hasProtection_bkn  = utils.v("BKIN_MODE",  motor) != "NONE" >
    <#local hasProtection_bkn2 = utils.v("BKIN2_MODE", motor) != "NONE" >


    <#if (!hasProtection_bkn && !hasProtection_bkn2) >
        <#return
...
<#-- External Comparator output have to be connected to the TIMER(Activate-Break-Input-2) so in case of overcurrent
 ...
 -->
    ...
    <#local bkin_channel= (hasProtection_bkn)?then("","2")>
...
<#-- it is correct tu map ACTIVE_HIGH to PULLDOWN and ACTIVE_LOW to PULLUP -->
    ...
    <#local active_break2_pin =
        ...
        , "${pinName}.Locked=true"
        , "${pinName}.Mode=Activate-Break-Input-${bkin_channel}"
