...
-->
<#macro oTemp_Bus_CurntSense_and_Bemf>

    <@ui.boxed "BUS, TEMPERATURE, CURRENTS SENSING and BEMF" >
...
<#macro tb_IP_section section>
    ...
    <@ui.box "${section.ip} ${section.title} Sensing" '' '.' />
...
<#import "../../MC_task/utils/mcu_pin_setting_mng.ftl" as cs >
...
<#function tb_required_conversions motor>
    ...
    <#local is_phase_enabled
         = utils.v("PHASE_CURRENTS_READING", motor)>
...
<#--#################################################################
 #  <define key="PHASE_CURRENTS_ADC"          value="ADC1"          />   #
 #  <define key="PHASE_CURRENTS_CHANNEL"      value="ADC_Channel_5" />   #
 #  <define key="PHASE_CURRENTS_GPIO_PORT"    value="GPIOF"         />   #
 #  <define key="PHASE_CURRENTS_GPIO_PIN"     value="GPIO_Pin_4"    />   #
 #  <define key="PHASE_CURRENT_ADC_SAMPLING_TIME" value="61"            />   #
 ####################################################################-->
<#function currentSensorInfo m>
    <#local sampling_cycles = utils.mx_name("PHASE_CURRENTS_ADC_SAMPLING_CYCLE") >
<#--TEMP_FDBK_ADC-->
    <#return
    { "ip"              : utils.mx_name(utils.v("PHASE_CURRENTS_ADC"    , m))
    , "channel"         : utils.v("PHASE_CURRENTS_CHANNEL"     , m)?upper_case
    , "isChShared"      : false
    , "pin"             : ns_pin.name2(m,"PHASE_CURRENTS")
    , "sampling_time"   : utils.v("CURR_SAMPLING_TIME", m)
    , "sampling_cycles" : sampling_cycles?is_hash?then(sampling_cycles.value!0, 5)
    , "label"           : "M${m}_PHASE_CURRENTS"
    , "motor"           : m
    , "type"            : "PHASE_CURRENTS"
    }>
</#function>
...
<#function bemfFInfo m x>
    <#local sampling_cycles = utils.mx_name("BEMF_ADC_SAMPLING_CYCLE") >

    <#return
    { "ip"              : utils.mx_name(utils.v("PHASE_${x}_BEMF_ADC"    , m))
    , "channel"         : utils.v("PHASE_${x}_BEMF_CHANNEL"     , m)?upper_case
    , "isChShared"      : false
    , "pin"             : ns_pin.name2(m,"PHASE_${x}_BEMF")
    , "sampling_time"   : utils.v("BEMF_ADC_SAMPLING_TIME", m)
    , "sampling_cycles" : sampling_cycles?is_hash?then(sampling_cycles.value!0, 5)
    , "label"           : "M${m}_BEMF_${x}"
    , "motor"           : m
    , "type"            : "BEMF_${x}"
