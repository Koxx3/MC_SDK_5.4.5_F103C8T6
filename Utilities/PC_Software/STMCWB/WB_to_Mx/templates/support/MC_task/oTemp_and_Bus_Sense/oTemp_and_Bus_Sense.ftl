...
<#macro tb_IP_section section>
    ...
    <#if cr.use_Cortex()>
    <#-- <#local Cortex_adc  = ns_cortex_ip.collect_CortexM4_IP( section.ip + "\\:I")>-->
        <#import "../utils/cortexMx_mng.ftl" as ns_cortex_ip>
        <@ns_cortex_ip.collectCortexIP ip_stored/>
    </#if>
...
<#--I have to rewrite only the CHANNEL, the MODE must be remain with the old code.  CHANNEL code IN  -->
    <#if cr.use_Cortex()>
      <#local channel= "INP${ utils._last_word(sensor.channel)}">
    </#if>
