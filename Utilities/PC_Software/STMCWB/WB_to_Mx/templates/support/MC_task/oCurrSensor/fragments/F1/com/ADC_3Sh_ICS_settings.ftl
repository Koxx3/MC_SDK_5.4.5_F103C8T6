...
<#--<#assign get_settings = adc_config>
<#function adc_config motor >-->
<#assign title = "ADC">
<#function get_settings motor >
    ...
    <#local RecoveryScanConvMode_1 = ex2.ScanConvMode_recovery(adc1) >
    <#local RecoveryScanConvMode_2 = ex2.ScanConvMode_recovery(adc2) >

    <#local M1_sense = utils.current_sensing_topology(1)!"THREE_SHUNT" >
    <#local sense = utils.short_sense(M1_sense)>
    <#if sense == "3Sh">
        <#local cnvs_adc1 = [cnv_U, cnv_V, cnv_W]
                cnvs_adc2 = [cnv_U, cnv_V, cnv_W] >
    <#else>
        <#local cnvs_adc1 = [cnv_U]
                cnvs_adc2 = [cnv_V] >
    </#if>

    <#local gpios_adc1 = ap.register_ADC_and_generatePinDefinition(adc1, cnvs_adc1, timer_idx,  sense, false)
            gpios_adc2 = ap.register_ADC_and_generatePinDefinition(adc2, cnvs_adc2, timer_idx,  sense, true ) >
