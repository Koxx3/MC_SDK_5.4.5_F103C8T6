...
-->
   ...
   <#-- <#local MD_mcu = (utils._last_word(WB_MCU_TYPE)?upper_case !"") >-->
    <#local flag = (WB_MCU?upper_case == "STM32F103ZG")>
    <#local irqs =
        { "TIM1"     : flag?then( "TIM1_UP_TIM10"  , "TIM1_UP"  )
        , "TIM8"     : flag?then( "TIM8_UP_TIM13"  , "TIM8_UP"  )
        , "TIM1_BRK" : flag?then( "TIM1_BRK_TIM9"  , "TIM1_BRK" )
        , "TIM8_BRK" : flag?then( "TIM8_BRK_TIM12" , "TIM8_BRK" )
        } >
...
<#function cs_PWM_TIMER_patameters timer no_outs>
    ...
    <#local MD_mcu = (utils._last_word(WB_MCU_TYPE)?upper_case !"") >
    <#local trig = (MD_mcu == "HD")?then("RESET","UPDATE")>
    ...
    , "TIM_MasterOutputTrigger"                    : "TIM_TRGO_${trig}"
...
<#function timer_srcs motor>
    ...
    <#return
        ...
        + m_item("PWM_PERIOD_CYCLES"          , motor)
