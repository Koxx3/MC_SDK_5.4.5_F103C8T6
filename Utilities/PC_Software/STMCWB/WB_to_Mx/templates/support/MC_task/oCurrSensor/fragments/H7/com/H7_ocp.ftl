...
<#-- it is correct tu map ACTIVE_HIGH to PULLDOWN and ACTIVE_LOW to PULLUP -->
    ...
    <#local active_break_pin =
        [ "${pinName}.GPIOParameters=GPIO_PuPd,GPIO_Label,PinAttribute"
        , "${pinName}.GPIO_Label=M${motor}_OCP"
        , "${pinName}.GPIO_PuPd=GPIO_${pull}"
        , "${pinName}.Locked=true"
        , "${pinName}.Mode=Activate-Break-Input"
        , "${pinName}.PinAttribute=${WB_CORTEX_TYPE}"
        , "${pinName}.Signal=${pwm_timer}_BKIN"
        ]?join('\n')>

    <#return
        { "settings" : { "eXternal COMP connected to ${pwm_timer}_BKIN": active_break_pin }
        , "GPIOs"    : []
        }>
</#function>
...
<#--Add the Cortex config for IP - Call the Macro  CortexMx-->
    <#import "../../../../utils/cortexMx_mng.ftl" as ns_cortex_ip>
    <@ns_cortex_ip.collectCortexIP comp/>
    <#return
        { "name"  : comp
        , "phase" : phase
        , "nic"   : x
...
<#--Add the Cortex config for IP - Call the Macro  CortexMx-->
    <#import "../../../../utils/cortexMx_mng.ftl" as ns_cortex_ip>
    <@ns_cortex_ip.collectCortexIP dac/>
