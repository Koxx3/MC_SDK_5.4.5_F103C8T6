...
<#function opamp_srcs motor who>
    ...
    <#else>
        ...
        <#--<#local opamp_mode = feedback_net_filtering?then("PGA Connected-INVERTINGINPUT_IO0_BIAS","PGA Not Connected" )> cube_Ver < 6.0
            <#local opamp_mode = feedback_net_filtering?then("PGA Internally connected_IO0_BIAS","PGA Not Connected" )      cube_Ver >=6.0 -->
            <#local opamp_mode = feedback_net_filtering?then(pga_Type_Connection(),"PGA Not Connected" )>
        <#else>
...
<#function isCube_6>
    ...
    <#return (ver?number >= 6)?then(true,false)>
</#function>

<#function pga_Type_Connection >
    <#return ((isCube_6())?then("PGA Internally connected_IO0_BIAS", "PGA Connected-INVERTINGINPUT_IO0_BIAS"))/>
</#function>
...
<#--, "VINM", concatenate(phase  , (opamp.mode == "Standalone")?then("OPAMP_GAIN", "OPAMP_INTGAIN") )-->
    ...
    <#local add_zero = (isCube_6() && (pin == "VINM"))?then("0", "")>
    <#local opamp_mode = ((pin == "VOUT") && (WB_USE_3OPAMPS!false) && isCube_6())?then("", "${opamp.mode}")>
    <#return
        { "name"   : pinName
        , "label"  : pinLabel
        , "signal" : "${opamp.name}_${pin}${add_zero}"
        , "mode"   : opamp_mode
