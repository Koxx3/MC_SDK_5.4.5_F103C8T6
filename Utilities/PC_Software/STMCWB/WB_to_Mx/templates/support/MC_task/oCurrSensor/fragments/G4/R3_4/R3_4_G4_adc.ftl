...
<#function cs_ADC_settings motor internal_opamps=false >
    ...
    <#if WB_USE_3OPAMPS!false>
        <#local adc1_cnvs = fp.map_ctx(motor, info.adc_info, ["U","W"][0..1] )
                adc2_cnvs = fp.map_ctx(motor, info.adc_info_internal_opamp, ["V","W"][0..1] )
        >
    <#--adc2_cnvs = fp.map_ctx(motor, info.adc_info, ["V"][0..*1] )-->

    <#-- ADC2 Channel_18 OPAMP3 Virtual Pin -->
            <#local sig_mode_s  =
            { "OPAMP_INTERNAL3" : "${adc2?upper_case}_OPAMP_INTERNAL3"
            , "OPAMP_INTERNAL3" : "IN-OPAMP3"}>
            <#list sig_mode_s as signal, mode>
                <#local v_signal = "${adc2?upper_case}_${signal}" >
...
-->
    ...
    <#return
        { "settings" :
            ...
            , "${adc2} Channel_18 OPAMP3 Virtual Pin" : fp.flatten(vps?values)?join("\n")
            }
        , "GPIOs"    : adc1_gpios
                     + adc2_gpios
        }>
...
<#function register_ADC_and_generatePinDefinition adc infs timer internal_opamps, sense>
    ...
    <#if adc =="ADC2" && (WB_USE_3OPAMPS!false) >
    <#--Non bisogna generare il pin per il Channel W (infs[1]) perche' Opamp3_Out e'collegata internamete ADC2 Channel_18 OPAMP3 Virtual Pin-->
    <#-- ADC2 Channel_18 OPAMP3 Virtual Pin  (Vedi -> Function(cs_ADC_settings) definita sopra -->
    <#-- VP_ADC2_OPAMP_INTERNAL3.Mode=IN-OPAMP3              -->
    <#--VP_ADC2_OPAMP_INTERNAL3.Signal=ADC2_OPAMP_INTERNAL3  -->
        <#return fp.map_ctx(ctx, ap.cs_create_adc_pins, [infs[0]])>
    <#else>
