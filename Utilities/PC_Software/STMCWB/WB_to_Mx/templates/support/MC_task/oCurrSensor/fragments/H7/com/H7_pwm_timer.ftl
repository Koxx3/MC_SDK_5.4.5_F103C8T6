...
<#--Add the Cortex config for IP - Call the Macro  CortexMx-->
    ...
    <@ns_cortex_ip.collectCortexIP timer_name/>
...
<#function cs_timer_irq ip>
   ...
   <#-- <#return ns_ip.ip_irq( irqs[ip], [true, 4, 1, true, false, false, true]) >-->
    ...
    <#local set_irq_tim_brk = rr["SET_IRQ_TIM_BRK"]!([true, 4, 0, true, false, false, true]) >
    <#return ns_ip.ip_irq( irqs[ip], set_irq_tim_brk) >
...
<#function cs_PWM_TIMER_patameters timer no_outs motor>
    ...
    , "BreakPolarity"    : "TIM_BREAKPOLARITY_HIGH" <#--"TIM_BREAKPOLARITY_${utils._last_word( timer.OVERCURR_FEEDBACK_POLARITY ) }"-->
    , "BreakState"      : "TIM_BREAK_${ (timer.BKIN_MODE=='NONE')?then('DISABLE', 'ENABLE')  }"


    ,"SourceBRKDigInputPolarity":"TIM_BREAKINPUTSOURCE_POLARITY_${utils._last_word( timer.OVERCURR_FEEDBACK_POLARITY ) }"

    <#--    Breakin2 non disponibile per la nucleo H7. Da verificare quando inseriremo la eval.-->
    <#--,"SourceBRK2DigInput"       : "TIM_BREAKINPUTSOURCE_${utils._last_word( timer.OVERCURR_FEEDBACK_POLARITY ) }"-->
