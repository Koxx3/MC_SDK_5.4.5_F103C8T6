...
<#function cs_adc_parameters timer sense cnvs use3opm>
    <#local tmr_idx = utils._last_char( timer ) >

    <#local InjectedConvMode   = "None" >
    <#local trig_suffix = use3opm?then('_CC4','_TRGO')>

    <#local ExternalTrigInjecConv = "ADC_EXTERNALTRIGINJEC_T${tmr_idx}${trig_suffix}" >

    <#local ExternalTrigInjecConvEdge = "ADC_EXTERNALTRIGINJECCONV_EDGE_${ (sense == '1Sh')?then('RISINGFALLING','RISING') }" >
