...
<#function opamp_srcs motor who>
    ...
    <#local i                 = ["",    "1",    "2"][who]>
    <#local f_opamp_vinp_gpio = [opamp_vinp, opamp_1_vinp, opamp_2_vinp][who]>
    ...
    <#else>
        <#local opamp_mode = feedback_net_filtering?then(
                             "PGA Connected",
                             "PGA Not Connected" )>
    </#if>
...
<#--Add the Cortex config for IP - Call the Macro  CortexMx-->
    ...
    <@ns_cortex_ip.collectCortexIP opamp_name/>
    ...
    <#if opamp.OPAMP_TYPE_GAIN == "INTERNAL" >
        ...
        <#local ip_params = {"PgaGain" : "OPAMP_PGA_GAIN_${pga_gain}"} + ip_params>
    </#if>
