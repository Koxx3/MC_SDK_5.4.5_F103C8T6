...
<#function cs_ADC_settings motor adc>
    <#local inf = info.adc_info(motor, "x") >

    <#local gpios = fp.map_ctx({"adc": adc}, adc_pin.cs_create_adc_pins, [inf]) >

    <#local sectionTitle = "${adc} settings for Current Sensing" >
    <#local adc_settings  = ns_ip.IP_settings(sectionTitle, adc, G0_ADC_cs_parameters() ) >
...
<#import "../../../../utils/meta_parameters.ftl" as meta >
<#function G0_ADC_cs_parameters>
    <#return meta.comment("G0 does not require additional ADC parameter settings for Current settings") >
</#function>
