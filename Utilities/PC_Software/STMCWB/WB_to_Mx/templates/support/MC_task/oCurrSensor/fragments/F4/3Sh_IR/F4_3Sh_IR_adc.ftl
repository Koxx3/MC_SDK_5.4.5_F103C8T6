...
<#function get_settings  motor >
    ...
    <#local opt_extras_reg_conv_1 = ex.add_extra_regular_cnv_if_needed(adc1, cnv_U)
            opt_extras_reg_conv_2 = ex.add_extra_regular_cnv_if_needed(adc2, cnv_U) >
            <#-- all the conversions have the same sampling -->

    <#local gpios_adc1 = ap.register_ADC_and_generatePinDefinition(adc1, [cnv_U, cnv_V, cnv_W], timer_idx, "3Sh_IR")
            gpios_adc2 = ap.register_ADC_and_generatePinDefinition(adc2, [cnv_U, cnv_V, cnv_W], timer_idx, "3Sh_IR") >
    ...
    <#return
        { "settings" :
                ...
                , "${adc?join(' and ')} IRQ" : ap.cs_adc_irq("ADC")
                }
