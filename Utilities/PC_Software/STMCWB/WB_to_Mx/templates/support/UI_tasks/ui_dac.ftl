...
<#function dac_settings>
    ...
    <#if DAC_EMULATED!false >
        ...
        <#if cr.use_Cortex()>
            ...
            <@ns_cortex_ip.collectCortexIP timer/>
        </#if>
    ...
    <#else>
        ...
        <#if cr.use_Cortex() >
            <#import "../../support/MC_task/utils/cortexMx_mng.ftl" as ns_cortex_ip>
            <@ns_cortex_ip.collectCortexIP dacName/>
        </#if>
...
<#function postponed_dac_settings dac dac_ch>
    ...
    <#if cr.use_Cortex()>
        ...
        { "DAC_Channel-DAC_OUT2" : "DAC_CHANNEL_2"}
        )>
    <#else>
        <#local dac_IP_settings = ns_ip.IP_settings("${dac} Settings for ${ DAC1 }", dac,
...
<#--
&lt;#&ndash;Viene chiamata quando DAC in Debug&ndash;&gt;
    <#local dac_IP_settings = ns_ip.IP_settings("${dac} Settings for ${ DAC1 }", dac,
       { "DAC_Trigger${ DAC_Trigger_key_suffix }" : "DAC_TRIGGER_SOFTWARE"
       , "DAC_OutputBuffer${ dac_setting_key_suffix }" : "DAC_OUTPUTBUFFER_DISABLE"
       }
...
-->
    ...
    <#local attr = cr.cortexPinAttribute(dac_pin)>

    <#return
        [ "${dac_pin}.GPIOParameters=GPIO_Label${attr.cortexLabel}"
        , "${dac_pin}.GPIO_Label=DBG_DAC_CH${dac_ch}"
        , "${dac_pin}.Locked=true"
        , "${dac_pin}.Signal=${sh_signal}"
        , "SH.${sh_signal}.0=${ signal },${ mode }"
        , "SH.${sh_signal}.ConfNb=1"
        , "${attr.cortexPinAttribute}"]>
</#function>
